...
export async function gacha(_token: string) {
  // const message: discord.Message = new discord.Message(
  //   discord.MESSAGE_TYPE.LOADING,
  // );
  ...
  const rating = stars(variables.role, pulled.media.popularity!);

  console.log(`pool length: ${pool.length}`);
  console.log(`pool variables: ${JSON.stringify(variables)}`);

  console.log(
    `${pulled.character.name.full} - ${rating}* - ${pulled.media.title.english}(${pulled.media.popularity}) - ${
...
function stars(role: anilist.CHARACTER_ROLE, popularity: number) {
