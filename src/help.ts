      ...
      .addEmbed(
        new discord.Embed()
          ...
          .setDescription([
            ...
            '',
            '__The more your party members are familiar with each other the better they preform__. This is called synergy, 5 party members from the same anime are better than 5 members from a mix of different media.',
            '',
            '`/party view` to view your current party.',
            '',
            '`/party assign [name] [spot?]` to assign a character you found to your party;',
            'leaving the `[spot]` parameter empty will assign the character to the first empty spot, or override spot #5 if there are no empty spots.',
            '',
            '`/collection stars` and `/collection media` can be used to browse your characters and select your potential party members.',
            '\u200B',
          ].join('\n'))
      ...
      .addEmbed(
        new discord.Embed()
          ...
          .setDescription([
            ...
            '',
            '- `/collection all` `/coll all` `/mm all`: _list user characters in bulks_',
            '- `/collection stars` `/coll stars` `/mm stars`: _list user your stars_',
            '- `/collection media` `/coll media` `/mm media`: _list user characters from a specific media_',
            ...
            '',
            '- `/profile` `/user`: _see user profile and party_',
            '',
